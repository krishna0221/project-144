...
def popular_movies():
    ...
    for index,value in output.iterrows():
        d1={
            ...
            'release_date':value['release_date'] or 'N/A' , 
            'rating':value['weighted_rating']
        }
        popular_movie.append(d1)
    return jsonify({
        'data':popular_movie,
        'status':'success'
    })
...
def recommended_movies():
    global liked_movies
    column_name=["original_title","poster_link","release_date","runtime","weighted_rating"]
    recommeded = pd.DataFrame(columns=column_name)
    for i in liked_movies:
        output=get_recommendations(i['original_title'])
        recommeded=recommeded.append(output)
    

    recommeded.drop_duplicates(subset=['original_title'],inplace=True)

    recommended_movie=[]
    for index,value in recommeded.iterrows():
        d1={
            'original_title':value['original_title'] , 
            'poster_link':value['poster_link'] , 
            'duration':value['runtime'], 
        ...
        recommended_movie.append(d1)
    return jsonify({
        'data':recommended_movie,
        'status':'success'
    })
